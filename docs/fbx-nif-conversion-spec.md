...
| The faces a thin collision box loses to quantisation | *Havok's floor, not a defect here.* A compressed mesh packs each chunk's vertices as 16-bit offsets counted in the `Error` step, which every one of the 8,188 shapes Skyrim ships sets to 0.001 — a millimetre. A shape only a few steps thick can lose faces to that: `civilwarmapflag01` is a box 0.49 game units (7mm) thick and comes back with all eight vertices but eight triangles of twelve — two whole faces gone, 12.2% of its surface, so a thing moving edge-on passes through where it used to collide. Rebuilding mopper at a hundredth of the step restores all twelve, which identifies the cause and is not a fix: the offsets are 16-bit, so a finer step trades range for resolution, and no vanilla file uses one. Not welding — welding would have taken vertices, and the same three meshes are byte-identical with the weld tolerance ten times finer | §5.7.3 |
