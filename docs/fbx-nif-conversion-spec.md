...
| The collision triangles a compressed mesh loses when Havok rebuilds it | *Open, and measured.* This is the same fault as the row below, and it is not confined to thin boxes: **277 of the 1,137 files with a compressed mesh in a 2,000-mesh sample lose triangles**, and 22 gain some. `falmerpgebalconybridge01` goes from 222 triangles to 90 — 59% of a bridge's collision surface. `civilwarrubble04` loses 187 of 882, `dlc2cliffbasaltoverhangbig` 272 of 2,016, `thiefstone` 14 of 202.<br><br>It tracks the chunking. Ours produces fewer chunks than Bethesda's in 233 of 1,137 files and never more — 14 against 7, 12 against 6, 4 against 1 — and the triangles that no longer fit a chunk either move to the big-triangle table or disappear. The material tables agree exactly in every one of those files, so the per-material split is not the cause, and `thiefstone` has a single material on both sides.<br><br>Not the builder settings either: mopper sets what `HKXWrangler.cpp:3349` sets, and the pass count, welding and tolerance were each tried against the box below. What has not been tried is how the geometry is *fed* — `beginSubpart`/`addGeometry` boundaries decide chunking, and Bethesda getting consistently more chunks from the same triangles suggests they fed it in pieces this merges. **The next step is to run ck-cmd on the same meshes and compare its chunking**, which settles whether this is the reference implementation's limit or this port's use of it | §5.7.3 |
