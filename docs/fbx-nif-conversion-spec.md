...
| The collision triangles a compressed mesh loses when Havok rebuilds it | *Open, and measured.* This is the same fault as the row below, and it is not confined to thin boxes: **277 of the 1,137 files with a compressed mesh in a 2,000-mesh sample lose triangles**, and 22 gain some. `falmerpgebalconybridge01` goes from 222 triangles to 90 — 59% of a bridge's collision surface. `civilwarrubble04` loses 187 of 882, `dlc2cliffbasaltoverhangbig` 272 of 2,016, `thiefstone` 14 of 202.<br><br>It tracks the chunking. Ours produces fewer chunks than Bethesda's in 233 of 1,137 files and never more — 14 against 7, 12 against 6, 4 against 1 — and the triangles that no longer fit a chunk either move to the big-triangle table or disappear. The material tables agree exactly in every one of those files, so the per-material split is not the cause, and `thiefstone` has a single material on both sides.<br><br>Not the builder settings either: mopper sets what `HKXWrangler.cpp:3349` sets, and the pass count, welding and tolerance were each tried against the box below.<br><br>**ck-cmd has now been run on the same meshes, and it splits the fault in two.** Its binary was round-tripped under wine — `exportfbx` then `importfbx` — and compared against both the source and this port:<br><br>| Mesh | Materials | Source | ck-cmd | This port |<br>| --- | --- | --- | --- | --- |<br>| `falmerpgebalconybridge01` | 1 | 4 chunks / 222 tris | 1 / 90 | **1 / 90** |<br>| `thiefstone` | 1 | 4 / 202 | 1 / 188 | **1 / 188** |<br>| `mrkmines01` | 1 | 14 / 824 | 7 / 709 | **7 / 709** |<br>| `civilwarrubble04` | **2** | 12 / 882 | **133 / 888** | 6 / 695 |<br><br>On a **single-material** mesh the two agree exactly, chunk for chunk and triangle for triangle, and both lose what Bethesda kept. That is the reference implementation's limit, reproduced faithfully, and not this port's to fix.<br><br>On a **multi-material** mesh they part company: ck-cmd keeps all 888 triangles across 133 chunks while this port keeps 695 across 6. That difference is this port's, and it is where §5.7.3's per-material split comes in — a mesh is cut into one geometry per material and each is built separately, where ck-cmd's chunk count says it feeds them as subparts of one build. **That is the piece worth fixing**, and it is now separable from the part that cannot be | §5.7.3 |
