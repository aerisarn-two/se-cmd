...
| A MOPP tree's origin margin | *A constant, but not the documented one.* nif.xml says the origin is "the minimum of all vertices in the packed shape along each axis, minus 0.1". No vanilla file uses 0.1. Of the 8,188 MOPP trees the game ships, **8,040 use 0.005**, 135 use the 0.05 that Havok's defaults produce and mopper and ck-cmd therefore write, and 13 put the origin *above* their own geometry by up to 0.034 — a tree whose frame does not contain the shape it indexes. The margin is Havok's rather than this port's: neither mopper nor ck-cmd sets the MOPP fit tolerances, and their defaults are not in the SDK source, so matching 0.005 means building against a candidate value and measuring what comes out | §5.7.3 |
