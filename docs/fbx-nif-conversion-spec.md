...
##### What no backend can build

mopper has two commands, `-msm` and `-ccm`, and NifMopp.dll has the same API: both build
a tree over **triangles**. A tree over a collection of primitives has leaves that are
child *indices* instead, and nothing available can produce one. Across all 22,047 vanilla
meshes:

| Tree wraps | Count | Treatment |
| --- | --- | --- |
| `bhkCompressedMeshShape` | 8,188 | Generated by the same call that chunks the mesh |
| `bhkListShape` | 86 | **Dropped**, with a warning — see below |
| `bhkNiTriStripsShape` | 26 | Generated by `-msm` from the rebuilt triangles |

The 86 pass through: the shape underneath comes back and the tree does not. Feeding a
tessellation of the primitives to `-msm` would produce a *valid-looking* tree whose leaves
index triangles the list shape does not have, and the engine would follow them into
children that do not exist. A missing index is slower; a wrong one is a crash. Listed in
§7.3 until mopper grows a command that takes child bounding volumes.
...
| A MOPP tree over a collection of primitives | 86 vanilla meshes wrap a `bhkListShape` in one. Every backend builds trees over triangles only, and a tree whose leaves index triangles a list shape does not have would send the engine into children that do not exist, so the tree is dropped with a warning and the shape below it comes back | §5.7.3 |
