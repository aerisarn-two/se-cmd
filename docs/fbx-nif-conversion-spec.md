...
**This port used to be *less* conservative than ck-cmd, and no longer is.** Generation ran
whenever the mesh had UVs (`FbxToNif.BuildShape`), and `TangentSpace.Generate` begins by
clearing both arrays — so a tangent space the FBX carried was read by `FbxMeshReader`,
kept through the vertex weld, and then discarded. ck-cmd would have kept it, and now so
does this. The two still disagree the other way round in the rare case of a mesh with no
UVs at all, where this port writes the tangents it read and ck-cmd writes none.

The stated reason — that FBX tangents "were split for FBX's own vertex layout" — did not
...
mapping is well defined. Regeneration was a choice, not a necessity.

It was the wrong choice for one real workflow. A normal map baked in Substance Painter or
xNormal is baked against a **MikkTSpace** basis; NifSkope's algorithm is not MikkTSpace,
so regenerating replaced the basis the map was authored against and the shading was wrong
in a way that reads as a bad bake rather than as a converter fault.

**And the corpus sweep saw it all along — it was almost all of what the sweep was
reporting.** The claim previously recorded here, that neither tool writes tangents *into*
an FBX and so a NIF → FBX → NIF trip regenerates at both ends regardless, is wrong:
`FbxMeshWriter` emits `LayerElementTangent` and `LayerElementBinormal` (§4), and
`FbxMeshReader` reads them back. The tangents made the round trip intact and were then
overwritten at the end of it.

Measured on four vanilla skinned meshes — `giant01`, `hmdaedra`, `dragon` and
`dragon_purple` — across 31,684 vertices, `Vertex`, `UV`, `Normal`, `Vertex Colors`,
`Bone Weights`, `Bone Indices`, `Unused W` and `Eye Data` all came back exact, while
`Tangent` and `Bitangent X` differed on **every single vertex**. Bethesda's tangents are
not NifSkope's. With the frame carried instead, all four round trip with no differing
...
Tangents above are now handled the same way, and the asymmetry that used to be recorded
here is gone: both are kept when the FBX carries them and computed only when it does not.
The reasoning for treating them differently — that a normal is geometry and survives a
round trip unchanged, while a tangent frame is a convention built for the FBX's own
vertex layout — did not hold up. The vertex layout is the same one on both sides of the
trip (§5.3.1).
...
#### 5.7.3 The MOPP tree is generated, never carried

A MOPP tree is Havok's own spatial index over the shape below it: a blob of bytecode the
engine walks to find which child a query touches. It is **regenerated on import**, from
the shape as actually rebuilt, and the bytes that came in are discarded.

That is a decision worth the measurement behind it. Regenerated code **never** matches
vanilla byte for byte:

| Trees over | Sampled | Identical |
| --- | --- | --- |
| `bhkCompressedMeshShape` | 15 | 0 — sizes differ by 1–2% |
| `bhkNiTriStripsShape` | 26 | 0 — sizes differ by ~50% |

So carrying the bytes would preserve only the *appearance* of fidelity: the code
describes one specific arrangement of triangles, a round trip refits every shape it
covers, and what came in describes a shape that no longer exactly exists. Generating
against what was rebuilt is the only way it can be right.

The same measurement found a defect it would otherwise have hidden. nif.xml puts the
quantisation in the code's own `Offset`, as its **W** — "the quantization factor is equal
to 256*256 divided by this number" — and NifSkope writes `Vector4(origin, scale)` there
and never touches the shape's `Scale`. This port had it the other way round, so every
...
a `bhkListShape` of primitives as readily as it indexes triangles. ck-cmd's `HKXWrangler`
does exactly that (`HKXWrangler.cpp:3256`): it builds the real Havok shape tree first,
then calls `buildCode` on `collection->getContainer()`.

What is missing without that is not a MOPP algorithm but **the shapes underneath it**. A
tree over a list has leaves that are child *indices*, so the children have to exist as
Havok shapes before anything can index them — which is why a tessellation cannot stand in
for one. It would produce a valid-looking tree whose leaves index triangles the list does
not have, and the engine would follow them into children that are not there. A missing
index is slower; a wrong one is a crash.

So mopper gained a third command. `-clm` reads a description of the shape tree —
`sphere`, `box`, `capsule`, `cylinder`, `convex`, and `transform` and `list`, which nest —
builds it, and calls the same `buildCode` on its container with chunk subdivision off, as
`HKXWrangler` sets it for PC. Its output matches `-msm` so a caller reads either the same
way, with the welding count zero: a list of primitives has no triangles to weld.
`MoppShapeWriter` produces the description from the rebuilt NIF blocks, whose lengths are
already in Havok units.

Across all 22,047 vanilla meshes:

| Tree wraps | Count | Built by |
| --- | --- | --- |
| `bhkCompressedMeshShape` | 8,188 | The same `-ccm` call that chunks the mesh |
| `bhkListShape` | 86 | `-clm`, from the primitives |
| `bhkNiTriStripsShape` | 26 | `-msm`, from the rebuilt triangles |

A shape the description does not cover makes the whole description null rather than a
...
| How a compressed mesh is cut into chunks | *By design, and measured.* A rebuilt compressed mesh is chunked differently from Bethesda's: over 835 such meshes in a 1,500-mesh sample, 230 come back with a different number of chunks, always fewer or equal — 14 against 7, 12 against 6, 4 against 1 — and the big-vertex table fills or empties accordingly.<br><br>**The shape is the same.** Counted as the tessellator reads it back, 829 of the 835 hold exactly the triangles they held; the chunking is where those triangles are *stored*, not which of them exist. Six differ, by 1 to 4 triangles each, and two of those six are `civilwarmapflag01` and `02` — the thin box of the row below.<br><br>ck-cmd was run over the same meshes under wine, `exportfbx` then `importfbx`. On a single-material mesh it produces the identical chunking to this port, chunk for chunk: `falmerpgebalconybridge01` 1 chunk, `thiefstone` 1, `mrkmines01` 7. On `civilwarrubble04`, which has two materials, it produces 133 chunks where this port produces 6 — and both hold all 1,354 triangles. So the chunk layout is one the reference implementation does not reproduce either, and nothing in the collision is lost by differing from it.<br><br>An earlier revision of this row claimed 277 files lost collision triangles and a bridge lost 59% of its surface. That was a counting error: triangles were totted up from chunk strip lengths and index counts rather than read back through the tessellator, and the arithmetic undercounts a chunk. The measurement above replaces it | §5.7.3 |
| The faces a thin collision box loses when Havok rebuilds it | *Open, and narrowed.* `civilwarmapflag01`'s collision is a box 0.049 × 0.007 × 0.063 m — twelve triangles over eight vertices — and comes back with eight triangles, 12.2% of its surface gone, so a thing moving edge-on passes through where it collided. What is handed to mopper is the whole box: twelve distinct triangles, no duplicates, six faces of correct winding. What mopper returns is one chunk of ten indices in a single strip — eight triangles, with no trailing list. Bethesda's own chunk for the same shape holds eighteen indices in strips of 10, 4 and 4: all twelve, at the same `Error = 0.001` every one of the game's 8,188 compressed shapes uses. So the encoding plainly represents this box and the fault is in the rebuild. Not welding: unchanged with `m_weldVertices` false, and welding would take vertices, which all eight survive. Not the weld tolerance, unchanged ten times finer. Not the stripper, unchanged at 0, 1 and 4 passes. Building at a hundredth of the quantisation step does restore all twelve, which says the step is involved without explaining how, since seven steps span the thin axis. **ck-cmd loses them too**: `HKXWrangler.cpp:3349-3363` sets the same `m_stripperPasses = 1`, `m_weldVertices = true`, `m_weldTolerance = 0.001f`, calls the same `createMeshShape(0.001f, MATERIAL_SINGLE_VALUE_PER_CHUNK)` and the same `beginSubpart`/`addGeometry`/`endSubpart`/`addInstance` on an identity transform, and finishes with the same `setRadius(0.005)`. Its one extra step, building an `hkpConvexVerticesConnectivity` to ask `isClosed()`, only chooses between the volume and surface inertia computations and never touches the geometry. So this is a limit of the reference implementation rather than a departure from it, and whatever Bethesda built these with was not `hkpCompressedMeshShapeBuilder` on these settings | §5.7.3 |
| A MOPP tree's origin margin | *A constant, but not the documented one.* nif.xml says the origin is "the minimum of all vertices in the packed shape along each axis, minus 0.1". No vanilla file uses 0.1. Of the 8,188 MOPP trees the game ships, **8,040 use 0.005**, 135 use the 0.05 that Havok's defaults produce and mopper and ck-cmd therefore write, and 13 put the origin *above* their own geometry by up to 0.034 — a tree whose frame does not contain the shape it indexes. The margin is Havok's rather than this port's, and not one of the MOPP fit tolerances: mopper was rebuilt with each of `m_absoluteFitToleranceOfTriangles`, `m_absoluteFitToleranceOfInternalNodes` and `m_absoluteFitToleranceOfAxisAlignedTriangles` set to 0.005 and the margin came out 0.050000 every time, to the last digit. Whatever fixes it is inside the compiler for this version of Havok and is not on the input struct. It cannot be corrected after the fact either — the tree's node bounds are quantised against the origin, so moving it without rebuilding shifts every bound with it | §5.7.3 |
| A convex hull's face planes | *Ours contain the corners; 306 files' do not.* A `bhkConvexVerticesShape` stores corners and half-spaces both, and Havok collides against the half-spaces. Half the game's 3,516 hulls come back with a different plane count, which looks like a fidelity gap until the file is asked to contain its own corners: **306 vanilla hulls do not**, by more than a hundredth of their span, and another 454 miss by less. The two arrays describe different bodies in the original. A rebuild that reproduces the corners exactly — 3,499 of 3,516 to within a thousandth of a span, median displacement zero — and fits planes around them therefore reads as "too big" against planes that never bounded those corners either. So `EveryVanillaConvexHullKeepsItsCorners` asks after the corners, which is the part both files agree is the shape, and what to do about the planes is open: matching them field for field would mean reproducing the fault | §5.7.1 |
| The chunking of a compressed mesh shape | *Congruent, not equal.* A `bhkCompressedMeshShapeData` does not store geometry; it stores chunks, each a cloud of millimetre offsets from an origin Havok chose, and Havok chooses again on the way back. So `Chunks`, `Num Chunks`, `Min` and `Offset` differ on a mesh that is perfectly intact, and no field comparison can say whether the *shape* survived. `EveryVanillaCollisionMeshKeepsItsShape` asks that instead, of the decoded triangles: surface area within 1%, bounding box within half a game unit, and no rebuilt vertex more than two units from anything in the source. 21,993 of 22,047 pass. Two things it deliberately does not ask, having found both measure the encoding: the triangle count, because welding takes slivers with it — `hhmainhallfgate01` loses two triangles of area 4.35 and 7.25 from a mesh totalling 837,117, changing the surface by 0.009% — and the source-to-rebuilt direction of the Hausdorff, because a vertex used only by a welded-away sliver has no counterpart and reports the distance to the next real one | §5.7.3 |
| Order within a skin partition's `Vertex Map` | *Accepted.* The **set** is always right — 860 of 860 partitions in a 600-mesh sample come back holding exactly the vertices they held — and only the sequence differs, along with the `Vertex Weights` and `Bone Indices` that are indexed by it. **Not the triangles.** They are in the shape's own numbering, as nif.xml's description of the map says outright -- it maps "the weight/influence lists in this submesh to the vertices in the shape being skinned", the weights and not the faces. Calling them "partition-local" here is the error that let the writer put them through the map for as long as it did; vanilla reaches index 878 from a 108-entry map in `0000282d` and 963 from a 142-entry one in `hair13`, which no local index could. The order follows no rule that can be re-derived: of those 860 maps, 317 are ascending, 229 are first appearance in the partition's own triangle list, and 314 are neither. Nor can it be recovered from the deformers, since a cluster lists only the vertices its own bone moves and no cluster holds the whole map. Reproducing it would mean carrying the sequence explicitly in a channel invented for it, which is the thing §5.2.3A exists to avoid, for an ordering the renderer does not read — it walks the map, and a map is as good in one order as another. So this is a byte-fidelity gap and not a behavioural one, and it is left open deliberately | §5.2.3A |
| The collision of `architecture/solitude/clutter/sbigplanter01` | *One mesh in 22,047.* Its compressed shape has two chunks, and the first is a near-flat sliver — 0.803 × 0.803 × **0.009** m, ten triangles, with a coincident vertex pair. Havok will not build a MOPP for that piece on its own, so the two-material split fails and the whole collision is dropped. The piece is not the problem in itself: the same geometry builds as one unsplit mesh, and the second chunk builds alone. It is being asked for as a separate geometry that does. Neither mopper's welding nor its chunk subdivision is involved — the piece fails with `m_weldVertices` off and with subdivision either way, both checked by rebuilding mopper. Falling back to one material would keep the collision and quietly make a planter one substance, which §5.7 declines to do on purpose | §5.7 |
...
| Tangent space | Generated from NifSkope's `spTangentSpace` (§5.3.1) rather than obtained from the FBX SDK, which also removes the need for ck-cmd's tangent/binormal swap. Like ck-cmd's SDK call, it keeps a tangent space the FBX already had and generates only a missing one — it used to regenerate over it, which was the field sweep's largest single source of difference. See §5.3.1. |
